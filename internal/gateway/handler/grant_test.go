...
func TestStorageGrantSuccess(t *testing.T) {
	...
	h := NewGrantHandler(config.Config{}, postgrest.New(postgrest.Config{URL: srv.URL, ServiceKey: "svc"}), nil)
...
func TestStorageGrantMissingParams(t *testing.T) {
	h := NewGrantHandler(config.Config{}, postgrest.New(postgrest.Config{URL: "http://127.0.0.1:1"}), nil)
	mux := http.NewServeMux()
	h.Register(mux)
