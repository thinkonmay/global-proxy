...
func TestPromoteQueryToken(t *testing.T) {
	req := httptest.NewRequest(http.MethodGet, "/v1/event/machine?token=raw-jwt", nil)
	PromoteQueryToken(req)
	if got := req.Header.Get("Authorization"); got != "Bearer raw-jwt" {
		t.Fatalf("Authorization = %q", got)
	}

	req2 := httptest.NewRequest(http.MethodGet, "/v1/event/machine?token=Bearer+prefixed", nil)
	PromoteQueryToken(req2)
	if got := req2.Header.Get("Authorization"); got != "Bearer prefixed" {
		t.Fatalf("Authorization = %q", got)
	}

	req3 := httptest.NewRequest(http.MethodGet, "/v1/event/machine", nil)
