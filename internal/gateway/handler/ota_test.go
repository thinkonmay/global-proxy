...
func TestOTAPublishReleaseSuccess(t *testing.T) {
	...
	h := NewOTAHandler(postgrest.New(postgrest.Config{URL: srv.URL, ServiceKey: "svc"}), "svc")
...
func TestOTAPublishReleaseRequiresServiceKey(t *testing.T) {
	h := NewOTAHandler(postgrest.New(postgrest.Config{URL: "http://127.0.0.1:1", ServiceKey: "svc"}), "svc")
	mux := http.NewServeMux()
	h.Register(mux)
