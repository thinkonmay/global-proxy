...
import (
	...
	coreusage "github.com/thinkonmay/global-proxy/api/pkg/usage"
)

// StartCollector runs the F06 metering tick loop in-process when enabled.
...
func (h *Handler) StartCollector(ctx context.Context, cfg *config.Config, log *slog.Logger) error {
	...
	dedup, err := coreusage.NewDedup(cfg.Metrics.RedisURL)
	if err != nil {
		_ = cache.Close()
		return fmt.Errorf("usage dedup: %w", err)
	}

	collector := coreusage.NewCollector(
		cache,
		coreusage.NewCatalog(h.pr, every),
		dedup,
		h.pr,
		h.eventBus,
		log,
